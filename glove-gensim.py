...
configs = Config()
glove_file=os.path.join(configs.wordVecRoot, configs.wordVecPath)
_, tokens, dimensions, _ = configs.wordVecPath.split('.')
num_lines = check_num_lines_in_glove(configs.wordVecPath)
dims = int(dimensions[:-1])

# Output: Gensim Model text format.
gensim_file=os.path.join(configs.wordVecRoot, configs.wordVecModelPath)
